...
const Cart = () => {
    ...
    return (
        <div className="container mx-auto px-4 py-8 min-h-screen">
            <h1 className="text-2xl sm:mt-8 md:text-4xl font-bold text-center mb-6 text-[var(--dark-green)]">Cart</h1>
                            ...
                            cartItems.map((item: CartItem) => (
                                <tr key={item.id} className="border-b">
                                    <td className="p-2 md:p-4 flex items-center space-x-2 md:space-x-4">
                                        <img src={item.image} alt={item.title} className="w-16 h-16 md:w-20 md:h-20 object-cover rounded-lg" />
                                    ...
                                    <td className="p-2 md:p-4">
                                        <div className="flex items-center space-x-1 md:space-x-2">
                                            ...
                                            <span className="text-center w-8 md:w-12">{item.quantity}</span>
                                            <button
                                                className="px-2 py-1 border rounded bg-[var(--light-green)] hover:bg-[var(--hover-green)] text-[var(--dark-green)]"
                                                onClick={() => handleQuantityChange(item.id, 1)}
                                            >
            ...
            <div className="w-full max-w-md bg-[var(--light-green)] shadow-lg rounded-lg p-4 md:p-6 mt-8 mx-auto">
                <h2 className="text-xl md:text-2xl font-semibold mb-4 text-[var(--dark-green)]">Cart Totals</h2>
