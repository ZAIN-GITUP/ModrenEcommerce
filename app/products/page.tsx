...
const SeeAll: React.FC = () => {
  ...
  const { data: fetchedProducts, error: fetchError, isLoading: fetching } = useGetProductsByCategoryQuery(selectedCategory || '');
      ...
      <div className="flex flex-col items-center space-y-4">
        ...
        <div className="categoryButtons flex flex-wrap justify-center gap-2 mb-6">
          <button
            ...
            className={`bg-cyan-400 text-cyan-600 px-4 py-2 rounded ${selectedCategory === '' ? 'bg-blue-700' : ''}`}
          >
            All
          </button>
          {['men\'s clothing', 'women\'s clothing', 'electronics', 'jewelery'].map((cat) => (
            <button
              ...
              className={`bg-cyan-400 text-cyan-600 px-4 py-2 rounded ${selectedCategory === cat ? 'bg-blue-700' : ''}`}
            >
              {cat}
            </button>
          ))}
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
          {currentProducts.map(product => (
            <div key={product.id} className="relative text-center bg-white rounded-lg shadow-lg p-6 group">
